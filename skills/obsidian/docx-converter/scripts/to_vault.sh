...
usage() {
	cat <<'EOF'
Usage:
  bash to_vault.sh <input.docx> [output-rel-path] [extra-args...]

Examples:
  bash to_vault.sh "/path/to/report.docx"
  bash to_vault.sh "/path/to/report.docx" "stock/调研笔记"
  bash to_vault.sh "/path/to/report.docx" "stock/交易笔记" --analyze
  bash to_vault.sh "/path/to/report.docx" --yes
...
if [ "$confirm" = true ]; then
	printf "\nExecution Preview\n"
	printf -- "- Skill: ob-docx-converter\n"
	printf -- "- Input docx: %s\n" "$input_docx"
	printf -- "- Vault root: %s\n" "$vault_root"
...
python3 "$core_script" "$input_docx" "$output_dir" "${extra_args[@]}"
