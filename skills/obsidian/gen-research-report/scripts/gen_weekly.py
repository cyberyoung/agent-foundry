#!/usr/bin/env python3
"""
gen_weekly.py — 根据年月生成研报周笔记框架

用法:
  python3 gen_weekly.py <年月> [options]
  python3 gen_weekly.py 202604
  python3 gen_weekly.py 202604 --dry-run
  python3 gen_weekly.py 202604 --vault-root /path/to/vault
"""
...
def build_week_content(week_start: date, week_end: date, week_num: int) -> str:
    ...
    return f"""---
title: 研报阅读{year}{month:02d}W{week_num}
...
# 研报阅读 {year}年{month:02d}月 W{week_num} ({week_start.strftime('%m-%d')} ~ {week_end.strftime('%m-%d')})
...
def main():
    parser = argparse.ArgumentParser(description="生成研报周笔记框架")
    ...
    output_dir = vault_root / "stock" / "调研笔记" / year_str / month_str
    ...
    for week_start, week_end, week_num in weeks:
        ...
        filename = f"研报阅读{year_month}-W{week_num}.md"
